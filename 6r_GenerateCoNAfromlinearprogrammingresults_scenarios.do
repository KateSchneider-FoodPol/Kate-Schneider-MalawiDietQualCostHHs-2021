...
Purpose: Take linear programming output to generate CoNA and nutrient and semi-elasticities
		for policy scenarios using revised household nutrient requirements 
		(defined by members 4 and above only).
...
cd "yourfilepath"
cap log close
log using "6r_scenarios_GenerateCoNA_SemiElasticities__`c(current_date)'", replace
***Log of Do File #6r "6r scenarios Transform Linear Programming Output into Variables"
...
use "CPIDataForCoNA_oldstata_s1`s'.dta", clear 
...
save "CPIDataForCoNA_foodcomp_s1`s'.dta", replace

* Read R result back to Stata
clear
insheet using "HH_LPResults_sharing_r_s1`s'.csv"
describe
sum
save "HH_LPResults_sharing_r_s1`s'.dta", replace

* Generate market number, year and month
use "HH+CPI_LPInput_sharing_r_s1`s'.dta", clear
describe
sum
...
merge 1:1 v1 using "HH_LPResults_sharing_r_s1`s'.dta"
drop v1 _merge
forvalues i=2/52 {
rename v`i' F`=`i'-1'
}
...
* Add food price data
append using "HH+CPI_LPInput_sharing_r_s1`s'.dta"
cap drop rel rhs
sum
describe
...
preserve
    * Generate total nutrient intakes per household diet
        merge m:1 food_no using "CPIDataForCoNA_foodcomp.dta", ///
        	keepusing(food_item food_group food_group_name MWI_food_group_name MWI_food_group)
	...
	* Save food level dataset
	save "HHCoNA_sharing_foodlevel_r_s1`s'.dta", replace
...
rename food_cost CoNA_sharing_s1`s'
label var CoNA_sharing_s1`s' "Cost of Nutrient Adaquacy - scenario 1`s'"

* Identify HH-market-months with no linear programming solution
sum if CoNA_sharing_s1`s'==0
replace CoNA_sharing_s1`s'=. if CoNA_sharing_s1`s'==0 
...
tempvar CoNA_nosolution
	gen `CoNA_nosolution'=1 if CoNA_sharing_s1`s'==.
	replace `CoNA_nosolution'=0 if CoNA_sharing_s1`s'!=. & `CoNA_nosolution'==.
...
save "HHCoNA_sharing_r_s1`s'.dta", replace
...
* Read R result back to Stata
clear
insheet using "HH_Shadowprices_sharing_r_s1`s'.csv"
describe
sum
...
save "HH_shadowprices_sharing_r_s1`s'.dta", replace
...
use "HHCoNA_sharing_s1`s'", clear
	sort case_id HHID y2_hhid y3_hhid data_round dateMY
		...
		replace y2_hhid="" if y2_hhid=="."
		replace y3_hhid="" if y3_hhid=="."
	merge 1:m v1 using "HH_shadowprices_sharing_r_s1`s'"
	drop _merge
...
preserve
	use "HH+CPI_LPInput_sharing_r_s1`s'", clear
		* Generate market price observation date
			cap drop dateMY
			gen dateMY=ym(year, month)
			format dateMY %tmCCYY-NN
	...
	keep v1 constraint_no constraint_type nutr_no rel rhs case_id HHID y2_hhid y3_hhid market_no dateMY
		save "HHNR_timeseries_r_s1`s'", replace
...
merge 1:1 v1 constraint_no constraint_type nutr_no dateMY case_id HHID y2_hhid y3_hhid market_no ///
	using "HHNR_timeseries_r_s1`s'", keepusing(rel rhs)
	drop _merge
	drop v1
...
// Save time series datasets
	...
	* Save applicable dates for time series of shadow prices
	...
	bys nutr_no: tab constraint_no, sum(semielasticity_sharing)
		save "/cluster/tufts/masterslab/kschne02/HHCoNA_ShadowPrices_r_s1`s'", replace
	...
	* * Keep applicable dates for time series dataset
	...
	use "HHCoNA_sharing_foodlevel_r_s1`s'", clear
	cap drop _*
	cap drop hhxround
	* Keep only relevant time series
	...
	tab2 year data_round
	drop if intake==0
	save "HHCoNA_foodlevel_r_s1`s'", replace
}
*/
forval s=6/6 {
...
// PART 1 // GENERATE CONA - SHARING
* Create food composition data to merge back in
use "CPIDataForCoNA_oldstata_s`s'.dta", clear 
...
save "CPIDataForCoNA_foodcomp_s`s'.dta", replace

* Read R result back to Stata
clear
insheet using "HH_LPResults_sharing_r_s`s'.csv"
describe
sum
save "HH_LPResults_sharing_r_s`s'.dta", replace

* Generate market number, year and month
use "HH+CPI_LPInput_sharing_r_s`s'.dta", clear
describe
sum
collapse (mean) nutr_no (first) market_no, by(hhxround case_id HHID y2_hhid y3_hhid year month data_round date_consump)
egen v1=seq() 
...
* Add linear programming food intakes 
merge 1:1 v1 using "HH_LPResults_sharing_r_s`s'.dta"
drop v1 _merge
forvalues i=2/52 {
rename v`i' F`=`i'-1'
}
...
* Add food price data
append using "HH+CPI_LPInput_sharing_r_s`s'.dta"
cap drop rel rhs
sum
describe
sort case_id HHID y2_hhid y3_hhid year month nutr_no
...
preserve
    * Generate total nutrient intakes per household diet
        merge m:1 food_no using "/cluster/tufts/masterslab/kschne02/CPIDataForCoNA_foodcomp.dta", ///
        	keepusing(food_item food_group food_group_name MWI_food_group_name MWI_food_group)
        drop if _merge==2 // foods not observed
        drop _merge
		    // Check for correct match from intake to food item 
	...
	* Save food level dataset
	save "HHCoNA_sharing_foodlevel_r_s`s'.dta", replace
...
* Generate CoNA (sharing)
collapse (sum) food_cost (first) market_no date_consump, by(case_id HHID y2_hhid y3_hhid data_round year month)
rename food_cost CoNA_sharing_s`s'
label var CoNA_sharing_s`s' "Cost of Nutrient Adaquacy - scenario `s'"

* Identify HH-market-months with no linear programming solution
sum if CoNA_sharing_s`s'==0
replace CoNA_sharing_s`s'=. if CoNA_sharing_s`s'==0 
...
* No solution?
tempvar CoNA_nosolution
	gen `CoNA_nosolution'=1 if CoNA_sharing_s`s'==.
	replace `CoNA_nosolution'=0 if CoNA_sharing_s`s'!=. & `CoNA_nosolution'==.
	tab `CoNA_nosolution', m
...
save "HHCoNA_sharing_r_s`s'.dta", replace
...
* Read R result back to Stata
clear
insheet using "HH_Shadowprices_sharing_r_s`s'.csv"
describe
sum
* Label variables (constraints in order)
ren v2 energy_eer
...
reshape long shadowprice, i(v1) j(constraint_no)
ren shadowprice shadowprice_sharing
save "HH_shadowprices_sharing_r_s`s'.dta", replace
...
use "HHCoNA_sharing_r_s`s'", clear
	sort case_id HHID y2_hhid y3_hhid data_round dateMY
		...
		replace y2_hhid="" if y2_hhid=="."
		replace y3_hhid="" if y3_hhid=="."
	merge 1:m v1 using "HH_shadowprices_sharing_r_s`s'"
	drop _merge
	lab def constraintno 1 "Price" ///
		2 "Energy_eer" ///
		3 "Carb_ear" ///
...
lab var shadowprice_sharing "Shadow price (nominal MWK) for 1 unit change in constraint"
preserve
	use "HH+CPI_LPInput_sharing_r_s`s'", clear
		* Generate market price observation date
			cap drop dateMY
			gen dateMY=ym(year, month)
			format dateMY %tmCCYY-NN
	...
	keep v1 constraint_no constraint_type nutr_no rel rhs case_id HHID y2_hhid y3_hhid market_no dateMY
		save "HHNR_timeseries_r_s`s'", replace
...
* Merge in nutrient requirements data
merge 1:1 v1 constraint_no constraint_type nutr_no dateMY case_id HHID y2_hhid y3_hhid market_no ///
	using "HHNR_timeseries_r_s`s'", keepusing(rel rhs)
	drop _merge
	drop v1
...
sort case_id HHID y2_hhid y3_hhid dateMY nutr_no constraint_no
save "HH_shadowprices_sharing_r_s`s'", replace

// Save time series datasets

	* Save applicable dates for time series of shadow prices
	use "HH_shadowprices_sharing_r_s`s'"
		// Generate an annual mean shadow price per household
		gen shadowprice_sharing_pd=shadowprice_sharing/7
		tempvar shadowprice_sharing365
			gen `shadowprice_sharing365'=shadowprice_sharing_pd*365
		egen shadowprice_sharing_annual=mean(`shadowprice_sharing365'), by(case_id HHID y2_hhid y3_hhid constraint_no year)
		gen semielasticity_sharing_pd=semielasticity_sharing/7
		tempvar semielasticity_sharing365
			gen `semielasticity_sharing365'=semielasticity_sharing_pd*365		
		egen semielasticity_sharing_annual=mean(`semielasticity_sharing365'), by(case_id HHID y2_hhid y3_hhid constraint_no year)
		gen elasticity_sharing_pd=elasticity_sharing/7
		tempvar elasticity_sharing365
			gen `elasticity_sharing365'=elasticity_sharing_pd*365
		egen elasticity_sharing_annual=mean(`elasticity_sharing365'), by(case_id HHID y2_hhid y3_hhid constraint_no year)
			replace elasticity_sharing_annual=elasticity_sharing_annual*12
	tab2 year data_round
		drop if data_round==1 & year>=2013
		drop if data_round==2 & (year<2013 | year>2015)
		drop if data_round==3 & year<2016
	...
	* Keep applicable dates for time series dataset
	...
	use "HHCoNA_sharing_foodlevel_r_s`s'", clear
	cap drop _*
	cap drop hhxround
	* Keep only relevant time series
	drop if data_round==1 & year>=2013
	...
	tab2 year data_round
	drop if intake==0
	save "HHCoNA_foodlevel_r_s`s'", replace
	*/
}
