...
cd "yourfilepath"
cap log close
log using "06_GenerateCoNA_SemiElasticities_`c(current_date)'", replace
***Log of Do File #6 "Transform Linear Programming Output into Variables"
...
clear
insheet using "HH_LPResults_sharing.csv"
describe
sum
save "HH_LPResults_sharing.dta", replace

* Generate market number, year and month
use "HH+CPI_LPInput_sharing.dta", clear
describe
sum
...
merge 1:1 v1 using "HH_LPResults_sharing.dta"
drop v1 _merge
forvalues i=2/52 {
rename v`i' F`=`i'-1'
}
...
* Add food price data
append using "HH+CPI_LPInput_sharing.dta"
cap drop rel rhs
sum
describe
...
preserve
	...
	* Save food level dataset
	save "HHCoNA_sharing_foodlevel.dta", replace
	...
	* Save total nutrient intakes dataset to confirm CoNA diet meets needs
	...
	sum total*
	save "HHCoNA_sharing_totalnutrients.dta", replace
...
save "HHCoNA_sharing.dta", replace
...
* Read R result back to Stata
clear
insheet using "HH_Shadowprices_sharing.csv"
describe
sum
...
save "HH_shadowprices_sharing.dta", replace

* Merge shadow prices into household identifiers & CoNA
use "HHCoNA_sharing", clear
	sort case_id HHID y2_hhid y3_hhid data_round dateMY
		...
		replace y2_hhid="" if y2_hhid=="."
		replace y3_hhid="" if y3_hhid=="."
	merge 1:m v1 using "HH_shadowprices_sharing"
	drop _merge
...
preserve
	use "HH+CPI_LPInput_sharing", clear
		* Generate market price observation date
			cap drop dateMY
			gen dateMY=ym(year, month)
			format dateMY %tmCCYY-NN
...
merge 1:1 v1 constraint_no constraint_type nutr_no dateMY case_id HHID y2_hhid y3_hhid market_no ///
	using "HHNR_timeseries", keepusing(rel rhs)
	drop _merge
	drop v1
...
// Save time series datasets
	...
	* Save applicable dates for time series of shadow prices
	...
	bys nutr_no: tab constraint_no, sum(semielasticity_sharing)
		save "HHCoNA_ShadowPrices", replace
