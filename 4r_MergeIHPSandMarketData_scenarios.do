...
Purpose: Merge IHPS and Market data and prepare for linear programming for policy 
			scenarios with revised shared nutrient requirements (defined by
			members age 4 and above only)
...
cd "yourfilepath"
log using "04r_scenarios MergeIHPSandMarketDataLPInput_`c(current_date)'", replace
...
saveold "HH+CPI_LPInput_sharing_r_s1`s'.dta", replace
}
...
saveold "HH+CPI_LPInput_sharing_r_s`s'.dta", replace
}
