...
cd "yourfilepath"
cap log close
log using "06_scenarios_GenerateCoNA_SemiElasticities__`c(current_date)'", replace

***Log of Do File #6 "6 scenarios Transform Linear Programming Output into Variables"
...
clear
insheet using "HH_LPResults_sharing_s1`s'.csv"
describe
sum
save "HH_LPResults_sharing_s1`s'.dta", replace

* Generate market number, year and month
use "HH+CPI_LPInput_sharing_s1`s'.dta", clear
describe
sum
...
merge 1:1 v1 using "HH_LPResults_sharing_s1`s'.dta"
drop v1 _merge
forvalues i=2/52 {
rename v`i' F`=`i'-1'
}
...
* Add food price data
append using "HH+CPI_LPInput_sharing_s1`s'.dta"
cap drop rel rhs
sum
describe
...
preserve
	...
	* Save food level dataset
	save "HHCoNA_sharing_foodlevel_s1`s'.dta", replace
...
save "HHCoNA_sharing_s1`s'.dta", replace
...
* Read R result back to Stata
clear
insheet using "HH_Shadowprices_sharing_s1`s'.csv"
describe
sum
...
save "HH_shadowprices_sharing_s1`s'.dta", replace
...
use "HHCoNA_sharing_s1`s'", clear
	sort case_id HHID y2_hhid y3_hhid data_round dateMY
		...
		replace y2_hhid="" if y2_hhid=="."
		replace y3_hhid="" if y3_hhid=="."
	merge 1:m v1 using "HH_shadowprices_sharing_s1`s'"
	drop _merge
...
preserve
	use "HH+CPI_LPInput_sharing_s1`s'", clear
		* Generate market price observation date
			cap drop dateMY
			gen dateMY=ym(year, month)
			format dateMY %tmCCYY-NN
	...
	keep v1 constraint_no constraint_type nutr_no rel rhs case_id HHID y2_hhid y3_hhid market_no dateMY
		save "HHNR_timeseries_s1`s'", replace
...
merge 1:1 v1 constraint_no constraint_type nutr_no dateMY case_id HHID y2_hhid y3_hhid market_no ///
	using "HHNR_timeseries_s1`s'", keepusing(rel rhs)
	drop _merge
	drop v1
...
save "/cluster/tufts/masterslab/kschne02/HH_shadowprices_sharing_s1`s'", replace

// Save time series datasets

	* Save applicable dates for time series of shadow prices
	use "HH_shadowprices_sharing_s1`s'"
	...
	* Keep applicable dates for time series dataset
	...
	use "HHCoNA_sharing_foodlevel_s1`s'", clear
	cap drop _*
	cap drop hhxround
	* Keep only relevant time series
	...
	tab2 year data_round
	drop if intake==0
	save "HHCoNA_foodlevel_s1`s'", replace
}
...
* Read R result back to Stata
clear
insheet using "HH_LPResults_sharing_s`s'.csv"
describe
sum
save "HH_LPResults_sharing_s`s'.dta", replace

* Generate market number, year and month
use "HH+CPI_LPInput_sharing_s`s'.dta", clear
describe
sum
collapse (mean) nutr_no (first) market_no, by(hhxround case_id HHID y2_hhid y3_hhid year month data_round date_consump)
egen v1=seq() 
...
* Add linear programming food intakes 
merge 1:1 v1 using "HH_LPResults_sharing_s`s'.dta"
drop v1 _merge
forvalues i=2/52 {
rename v`i' F`=`i'-1'
}
...
* Add food price data
append using "HH+CPI_LPInput_sharing_s`s'.dta"
cap drop rel rhs
sum
describe
sort case_id HHID y2_hhid y3_hhid year month nutr_no
...
preserve
	...
	* Save food level dataset
	save "HHCoNA_sharing_foodlevel_s`s'.dta", replace
...
save "/cluster/tufts/masterslab/kschne02/HHCoNA_sharing_s`s'.dta", replace
...
describe
sum
save "HHCoNA_sharing_s`s'.dta", replace

// PART 2 // GENERATE NUTRIENT SHADOW PRICES - SHARING
* Read R result back to Stata
clear
insheet using "HH_Shadowprices_sharing_s`s'.csv"
describe
sum
* Label variables (constraints in order)
ren v2 energy_eer
...
reshape long shadowprice, i(v1) j(constraint_no)
ren shadowprice shadowprice_sharing
save "HH_shadowprices_sharing_s`s'.dta", replace
...
use "HHCoNA_sharing_s`s'", clear
	sort case_id HHID y2_hhid y3_hhid data_round dateMY
		...
		replace y2_hhid="" if y2_hhid=="."
		replace y3_hhid="" if y3_hhid=="."
	merge 1:m v1 using "HH_shadowprices_sharing_s`s'"
	drop _merge
	lab def constraintno 1 "Price" ///
		2 "Energy_eer" ///
		3 "Carb_ear" ///
...
lab var shadowprice_sharing "Shadow price (nominal MWK) for 1 unit change in constraint"
preserve
	use "HH+CPI_LPInput_sharing_s`s'", clear
		* Generate market price observation date
			cap drop dateMY
			gen dateMY=ym(year, month)
			format dateMY %tmCCYY-NN
	...
	keep v1 constraint_no constraint_type nutr_no rel rhs case_id HHID y2_hhid y3_hhid market_no dateMY
		save "HHNR_timeseries_s`s'", replace
...
* Merge in nutrient requirements data
merge 1:1 v1 constraint_no constraint_type nutr_no dateMY case_id HHID y2_hhid y3_hhid market_no ///
	using "HHNR_timeseries_s`s'", keepusing(rel rhs)
	drop _merge
	drop v1
...
sort case_id HHID y2_hhid y3_hhid dateMY nutr_no constraint_no
save "HH_shadowprices_sharing_s`s'", replace

// Save time series datasets

	* Save applicable dates for time series of shadow prices
	use "HH_shadowprices_sharing_s`s'"
		// Generate an annual mean shadow price per household
		gen shadowprice_sharing_pd=shadowprice_sharing/7
	tab2 year data_round
		drop if data_round==1 & year>=2013
	...
	* * Keep applicable dates for time series dataset
	...
	use "HHCoNA_sharing_foodlevel_s`s'", clear
	cap drop _*
	cap drop hhxround
	* Keep only relevant time series
	drop if data_round==1 & year>=2013
	...
	tab2 year data_round
	drop if intake==0
	save "HHCoNA_foodlevel_s`s'", replace
}
