...
cd "$analysis"
cap log close
log using "02r_HH Nut Reqts_`c(current_date)'", replace
***Log of Do File #2 "02r_Household Nutrient Requirements"
di "`c(current_date)' `c(current_time)'"
di "Excluding 3 year olds from defining share household requirements"

// PART 1 // USE DRIS PREPARED IN DO FILE 2
...
* Note the ULs/CDRR and AMDRs are used for the calculation of least cost diets only
...
gen EAR_sharing=.
	...
	replace EAR_sharing=ear_perweek if (inlist(age_sex_grp,2,3,4,5)) & ear_perweek!=.
	//Check 
...
* Generate new UL - Goal is to take the lowest UL/kcal of any age-gender group in the hh
	...
	* Problem - some ULs < EAR - generate min, mean and max for flexibility
	preserve
		replace ul_perkcal=. if inlist(age_sex_grp,2,3,4,5)
	...
	// Generate new UL for the household sharing level
		...
		replace UL_sharing=ul_perweek if (ageinmonths<=23 & age_sex_grp!=1) | inlist(age_sex_grp,4,5)
...
save HHNeeds_IndivLevel_r, replace

* Generate Max and Min AMDR PCT kcals at household level before collapse
cap drop _*
preserve
replace amdr_lower_pctkcal=. if inlist(age_sex_grp,1,2,3,4,5)
replace amdr_upper_pctkcal=. if inlist(age_sex_grp,1,2,3,4,5)
...
gen amdrlow_toddler_perweek=amdr_lower_perweek if (inlist(age_sex_grp,2,3,4,5) & ageinmonths!=.)
gen amdrup_toddler_perweek=amdr_lower_perweek if (inlist(age_sex_grp,2,3,4,5) & ageinmonths!=.)
	// Keep toddler energy needs to recalculate the rest of household's AMDRs below
	gen energytoddler=ear if (nutr_no==2 & (inlist(age_sex_grp,2,3,4,5) & ageinmonths!=.))
...
save HHNeeds_HHLevel_r, replace
...
tabstat ear ul amdr_lower amdr_upper, by(nutr_no) stats(mean sd min max count) c(s)
	...
	save HHdata_tomerge_sharing_r, replace
	* Save in format that is readable by older versions of Stata
	...
	saveold HHdata_tomerge_sharing_oldstata_r, replace
...
drop if age_sex_grp==1
	...
	*/
	...
	save HHdata_tomerge_targeting_r, replace
	* Save in format that is readable by older versions of Stata
		// Stata 13 is available on Tufts high power cluster computer with interactive interface option
	saveold HHdata_tomerge_targeting_oldstata_r, replace
